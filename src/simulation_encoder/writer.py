...
class Writer:
    ...
    def write_emulation_results(self, model_type: str, target: str, model_params: dict[str, float], r2: float) -> None:
    ...
    def write_encoded_data(self, model_name: str, encoded_data: dict[str, pd.DataFrame]) -> None:
        ...
        for key, data in encoded_data.items():
            data.to_csv(os.path.join(model_path, f"encoded_data/{key}.csv"), index=False)
