...
def gastruloid_binary_label(sample_id: str) -> int:
    ...
    """
    if sample_id.startswith("array1"):
        return 0
...
def load_latent_train_val_test(
    ...
    split_mode=csv uses the CSV split column
    split_mode=random redraws stratified splits
    """
...
def _fit_pca_on_train(
    ...
    """Fit PCA on training features (all components retained)."""
...
def _pca_transform_dataframes(
    ...
    *dfs: pd.DataFrame,
    ...
    for df in dfs:
        arr = pca.transform(df.values)
        out.append(pd.DataFrame(arr, columns=pc_cols, index=df.index))
    return out


def _pca_transform_sequence_loader(loader: "SequenceLoader") -> PCA:
    """
    ...
    for sid in list(loader.sequences):
        loader.sequences[sid] = pca.transform(loader.sequences[sid])
    return pca
...
def run_study_point_emergent_regression(
    ...
    if isinstance(emergent_property_name, str):
        target_col = emergent_property_name
    else:
        target_col = emergent_property_name[0]
    emergent_lookup = emergent_target_lookup_from_csv(emergents_csv, target_col)

    reg_list = list(regressor_types)
    allowed_models = {str(x) for x in encoder_model_names} if encoder_model_names else None
    ...
    for model_name, dataset_name, csv_path in iter_encoded_csv_paths(study_dir):
        ...
        X_train, y_train, X_val, y_val, X_test, y_test, ex_meta = Xt
        pca = _fit_pca_on_train(X_train)
        X_train, X_val, X_test = _pca_transform_dataframes(pca, X_train, X_val, X_test)
...
def run_study_trajectory_classification(
    ...
    for encoder_name, dataset_name, csv_path in iter_encoded_csv_paths(study_dir):
        if allowed_models is not None and encoder_name not in allowed_models:
            continue
        ...
        _pca_transform_sequence_loader(loader)
        n = loader.num_dims
