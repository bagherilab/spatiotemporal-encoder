...
def visualize_results(results, output_dir: str) -> None:
    ...
    pivot_df = df.pivot_table(index="encoder", columns="model", values="r2_score", aggfunc="mean")
...
def create_loaders(
    results_path: str, time_point_idx: int | None = None
) -> dict[str, dict[str, PointLoader]]:
