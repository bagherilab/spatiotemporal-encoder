    ...
    if continuous_params:
        for param, param_info in continuous_params.items():
            if param_info['search'] == 'linear':
                values = np.linspace(param_info['range'][0], param_info['range'][1], PARAM_RESOLUTION).tolist()
            elif param_info['search'] == 'log':
                values = np.logspace(np.log10(param_info['range'][0]), np.log10(param_info['range'][1]), PARAM_RESOLUTION).tolist()
